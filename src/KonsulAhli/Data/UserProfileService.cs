...
using KonsulAhli.Models;
using GemBox.Spreadsheet;
using System.Drawing;
using KonsulAhli.Tools;

namespace KonsulAhli.Data
{
    public class UserProfileService : ICrud<UserProfile>
    {
        ...
        {
            if (db == null) db = new KonsulAhliDB();
            ...
            try
            {
                ...
                foreach (var row in worksheet.Rows)
                {
                    if (counter > 0)
                    {
                        ...
                        newUserProfile.Username = string.IsNullOrEmpty(row.Cells[10].Value?.ToString().Trim()) ? MailHelper.GenerateEmailFromName(newUserProfile.FullName.Trim(), "KonsulAhli.online") :  row.Cells[10].Value?.ToString().Trim();
